...
(define (compile program)
  (compile-scode (syntax&integrate program '((usual-integrations)) test-env)))

(define-test 'msort-bug
  ;; We don't actually use the local `get-key' procedure; somehow this
  ;; triggers a LIAR bug.
  (lambda ()
    (define program
      '(let ()
		 ...
		 (let ((middle (fix:quotient (fix:+ low high) 2)))
		   ...
		   (let merge ((p low) (p1 low) (p2 middle))
			 ...
			 (if (and (fix:< p1 middle)
				  (or (fix:= p2 high)
				      (not (pred (key (vector-ref temp p2))
						 (key (vector-ref temp p1))))))
			     (begin
    ...
    (expect-error
     (lambda ()
       (muffle-warnings (lambda () (compile program)))))))