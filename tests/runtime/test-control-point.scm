...
(define-test 'raw-generator
  (lambda ()
    (let ((frames
      ...
      (let ((end (system-vector-length reference-cp)))
	(let loop ((frames frames) (index 2))
	  (if (and (pair? frames)
		   (fix:< index end))
		...
		(do ((i 0 (fix:+ i 1))
		  ...
		  (assert-eqv (vector-ref frame i)
			      (system-vector-ref reference-cp j))))))))))

(define-test 'decoded-frames
  (lambda ()
    (let ((frames
	   (generator->list
	    (control-point->frame-generator reference-cp))))
      (for-each (lambda (frame)
		  (let* ((raw (cpoint-frame-raw frame))
			 (end (vector-length raw)))
		    (assert-eqv (cpoint-frame-length frame) end)
		    (for-each (lambda (i)
				(assert-eqv (cpoint-frame-elt frame i)
					    (vector-ref raw i)))
			      (iota end)))
		  (let ((return (vector-ref (cpoint-frame-raw frame) 0)))
		    (assert-eqv (cpoint-frame-return-address frame)
				return)
		    (assert-eqv (cpoint-frame-compiled-code? frame)
				(compiled-return-address? return))
		    (assert-eqv (cpoint-frame-return-code frame)
				(and (interpreter-return-address? return)
				     (return-address/code return)))))
		frames)
      (let ((cp (cpoint-frames->control-point frames)))
