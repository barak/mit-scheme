  ...
  (lambda ()
    ...
    (define person-name-comparator
      (make-product-comparator
       (make-wrapper-comparator person?
				person-last-name
				string-ci-comparator)
       (make-wrapper-comparator person?
				person-first-name
    ...
    (let* ((beatles (make-person "The" "Beatles"))
	   ...
	   (mnd
	    (make-book (make-person "William" "Shakespeare")
		       "A Midsummer Night’s Dream"))
	   (bob (make-cd (make-person "Bob" "Dylan") "Blonde on Blonde"))
	   (revolver (make-cd (make-person "The" "Beatles") "Revolver")))
      (assert-equal (sort (list abbey-road deutsche-grammatik sonnets mnd bob
				revolver)
			  (comparator-ordering-predicate item-comparator))
		    (list deutsche-grammatik mnd sonnets abbey-road revolver
			  bob)))
    ))